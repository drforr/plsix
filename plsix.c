/*
 * PL/six language handler
 *
 * Copyright © 2019 by Jeffrey Goff
 * Original Copyright © 2012 by Peter Eisentraut
...
write_to_tempfile(const char *data)
{
	...
	if ((tmpdir_envvar = getenv("TMPDIR")))
		snprintf(tempfile, sizeof(tempfile), "%s/plsix.XXXXXX", tmpdir_envvar);
	else
		strcpy(tempfile, "/tmp/plsix-XXXXXX");
 ...
 * The PL handler
 */
PG_FUNCTION_INFO_V1(plsix_handler);

Datum
plsix_handler(PG_FUNCTION_ARGS)
{
 ...
 * Validator function
 */
PG_FUNCTION_INFO_V1(plsix_validator);

Datum
plsix_validator(PG_FUNCTION_ARGS)
{
 ...
 * Inline handler
 */
PG_FUNCTION_INFO_V1(plsix_inline_handler);

Datum
plsix_inline_handler(PG_FUNCTION_ARGS)
{
